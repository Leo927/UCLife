// LPC palette tables — vendored from
// Universal-LPC-Spritesheet-Character-Generator/palette_definitions/{body,hair}/{body,hair}_ulpc.json.
// BODY['light'] / HAIR['orange'] are the LPC source palettes that every body
// / hair sheet is drawn against — recolor.ts swaps from those to the target.

import type { LpcBodyPalette, LpcHairPalette } from './types'

export const BODY_SOURCE: LpcBodyPalette = 'light'
export const HAIR_SOURCE: LpcHairPalette = 'orange'

export const BODY: Readonly<Record<LpcBodyPalette, readonly string[]>> = {
...
export const HAIR: Readonly<Record<LpcHairPalette, readonly string[]>> = {
...
export function getPalette(material: 'body' | 'hair', color: string): readonly string[] {
  const table = material === 'body' ? BODY : HAIR
  const colors = (table as Record<string, readonly string[]>)[color]
  if (!colors) {
    throw new Error(`LPC palette: unknown ${material} color "${color}"`)
  }
  return colors
}

export function getSourcePalette(material: 'body' | 'hair'): readonly string[] {
  return material === 'body' ? BODY[BODY_SOURCE] : HAIR[HAIR_SOURCE]
}
