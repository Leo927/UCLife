 ...
 * @returns {string}
 */
globalThis.getGeneticEyeColor = function(actor, side, eyePart = "iris") {
	if (eyePart === "iris") {
		if (actor.geneticQuirks.albinism === 2 && "albinismOverride" in actor) {
			return actor.albinismOverride.eyeColor;
		} else {
			if (side === "left" && typeof actor.geneticQuirks.heterochromia === "string") {
				return actor.geneticQuirks.heterochromia;
			} else {
				return actor.eye.origColor;
			}
		}
	} else if (eyePart === "pupil") {
		return actor.race === "catgirl" ? "catlike" : "circular";
	} else {
		return "white";
	}
};
