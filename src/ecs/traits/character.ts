...
// Crew-duty state and station assignment for a crew NPC. W4.1: reconcile
// assigns each roster member a station anchor (px, in the ship-interior
// world) from the flagship class's authored `crewStations` list; the
// crew-duty BT branch walks there when underway, else to the mess/quarters.
// `anchorX/Y` default to -1 = "unassigned" (no crewStations authored).
// `current` is the crew-duty branch's live decision — read by the smoke +
// UI. The crew hold their post at Action 'idle' (a standing watch, not a
// committed timed task), so they keep re-evaluating as the ship's state and
// the clock change. Not persisted — reconcileCrewAboard re-assigns on
// board / load.
