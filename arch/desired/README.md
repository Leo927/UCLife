...
`001_component_layers.puml` is authored as of Wave 4. The rest of the suggested set (`002`-`006`) is **not yet authored** — the next agent picking this up should not assume they exist. Wave 8 (multi-world Proxy explicit-`World` argument contract for save and tick paths) landed.
...
- **Wave 8 — multi-world Proxy explicit-`World` argument contract.** `src/save/index.ts`, `src/sim/loop.ts`, and `src/boot/saveHandlers/relations.ts` no longer import the `world` Proxy. Each resolves `getWorld(getActiveSceneId())` once at the entry point (saveGame / loadGame, the per-frame `frame()`, the relations snapshot/restore closures) and passes / shadows it as an explicit `World` local. `buildMeta` takes an explicit `World` parameter. Active-scene callers (UI, render, debug handles, procgen, conversations, portrait) keep using the Proxy — it's legitimate convenience there. Save / migrate / cross-scene tick paths can no longer hide which world they're iterating.
...
The desired diagrams pin the *current* shape after Waves 1-3 + 5 + 7 + 8. There are no remaining cleanup waves enumerated; any further refactor must re-open the design conversation first.
