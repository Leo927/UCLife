...
test('colony build path — charter, package, construct, interrupt, save/load', async ({ sim }) => {
  ...
  // 1. Charter — rep too low should be rejected.
  const rejectedCharter = await sim.page.evaluate(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    () => (window as any).__uclife__.colonyGrantCharter('efsf', 0),
  )
  ...
  // Charter with sufficient rep should be granted.
  const grantedCharter = await sim.page.evaluate(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    () => (window as any).__uclife__.colonyGrantCharter('efsf', 50),
  )
