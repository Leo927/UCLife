...
| **6.2.5.C** | `currentPropellant` + `currentAmmoByWeapon` + `currentLifeSupport` on the MS runtime instance; `propellantStorage` + `lifeSupportMinutes` as template stats. **Mid-combat resupply protocol** with full `baseResupplySec / hangarBoss.workPerformance / (1 + Σ mechanicCrewEfficiency) / resourceBoostMul` formula. **Per-ship-class `hangarDoors[]` authoring** + door queueing + spawn-at-door-position. **Stranded-MS recovery tug** (`computers + mechanics` skill check; occupies a door for the duration). The MS runtime entity itself, the per-MS terminal interactable, and the retrofit panel surface this depth attaches to all land at **6.2.5.A** (core diegetic shape) and **6.2.5.B** (multi-MS / pilot / depot scale) — see [fleet.md § 6.2.5 sub-phase slicing](fleet.md#625-sub-phase-slicing). |
