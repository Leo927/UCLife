...
| **On-ship hangar deck** (per ship with `hangarCapacity > 0`) | The walkable hangar of a ship in the fleet, working forward repair during sortie | Ship's **hangar boss** NPC (a hired role); ship's mechanic crew bustling around the deck; pilots stand near their assigned MS | Walk up to the hangar boss → forward repair-priority, inspect MS state, sortie loadout (pull-from-surface / unload-back); walk up to a pilot → reassign / talk. **Capped at `onShipRepairCap` (~80%); no refit, no assembly, no destroyed→ready** — those route to a surface depot hangar. The depth of work is what makes the depot worth the trip home; throughput on a battleship or tender can rival a small surface hangar in raw speed. |
