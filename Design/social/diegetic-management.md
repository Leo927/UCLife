...
|---|---|---|---|
| **Flagship bridge** | The walkable bridge of the player's current ship | Hired captains stand at their stations *here* when their ships are docked with the fleet; an empty station means "ship is detached" | Talk to a captain → set their ship's doctrine, give standing orders, swap captain assignment, scrap / mothball a ship via conversation. The bridge is the *operational* room — helm, war room, combat stations. Per-ship and per-fleet *management* lives next door at the captain's office. |
| **War room** | A holographic plot table on the flagship bridge | None — the artifact itself | **Active fleet composition** (drag ship tokens between formation and reserve tray → writes `IsInActiveFleet`); fleet-wide formation slot arrangement; DP commit for next engagement; route planning. Info-dense, the one allowed abstraction (see below). |
| **Captain's office** | A small office room aboard every player-controllable ship, adjacent to the bridge — for capital ships it's a separate authored room (Bright Noa's office on White Base); for small hulls it's a curtained nook off the bridge with the captain's chair, desk, and a comm console. Authored per ship class. | The captain themselves (player-aboard or hired captain) | **Pre-launch readiness summary** read off this ship's state (supply / fuel / crew slots filled / MS loaded / pilot assigned per MS) — the captain narrates what's missing; **crew auto-man delegation** ("man the rest from idle pool" — see below); **prisoner verbs** via comm panel when prisoners are aboard the flagship; **private subordinate conversations** via the comm panel relocated here from the bridge floor (the bridge is loud and public; private orders happen at the desk). One per ship; the *flagship's* captain's office hosts the fleet-wide management subset (subordinate captains via comm, fleet roster as projection). |
| **Hangar facility** (surface or drydock) | A player-owned hangar facility in the city (surface — fighters, mobile workers, MS, MA, small craft) or at an orbital POI (drydock — capital ships). State-owned hangars accept rentals as the early-game escape valve. | Hangar manager (a hired NPC, the verb surface for everything fleet-resting); hangar workers do repair / resupply throughput; pilots stand near their assigned units | Walk up to a vehicle → retrofit panel; walk up to a pilot → reassign / talk; walk up to the manager → receive-delivery, repair-priority, refit, assemble, scrap, **transfer-unit-to-other-hangar** (paid + delayed), inspect supply / fuel against storage cap. The hangar is the diegetic home of fleet inventory — see [facilities-and-ownership.md](facilities-and-ownership.md#hangar-facility-phase-62). |
| **Supply dealer's desk** | A job site inside AE (POC) or other industrial-compound facility classes (later phases) | Supply-dealer NPC (worker on the job site) | Order N supply / fuel → confirm target hangar → confirm amount; shipment arrives ~2 in-game days later. Fuel and supply are real, finite resources stored at each hangar against `supplyStorage` / `fuelStorage` caps. The dealer is the inventory-replenishment counterpart to the hangar manager (who *spends* supply, not buys it). Secretary's `bulkOrderSupply` verb is the late-game scale shortcut, filling every hangar to cap. |
| **On-ship hangar deck** (per ship with `hangarCapacity > 0`) | The walkable hangar of a ship in the fleet, working forward repair during sortie | Ship's **hangar boss** NPC (a hired role); ship's mechanic crew bustling around the deck; pilots stand near their assigned MS | Walk up to the hangar boss → forward repair-priority, inspect MS state, sortie loadout (pull-from-surface / unload-back); walk up to a pilot → reassign / talk. **Effective repair band is `[onShipRepairFloor, onShipRepairCap]` (default ~`[0.4, 0.8]`)** — units below floor are sidelined for the rest of the sortie, units above cap have to depot for full restoration. **No refit, no assembly, no destroyed→ready** — those route to a surface depot hangar. Both floor and cap are ship stats, modifiable by officer skill, damage state, faction research, and doctrine stance. The depth of work is what makes the depot worth the trip home; throughput on a battleship or tender can rival a small surface hangar in raw speed. |
...
| **Faction office** | A facility class the player can buy in any city; multiple are allowed | Secretary (worker on the job site) | Roster idle members and assign in batch; read the books; surface the day's warning loop; (post-creation) run diplomacy / war-declaration verbs as Phase-6.4 stand-ins. The secretary is a *consultative delegate*, not a wrapper around five tabs — anything denser routes to the relevant hub. Also hosts the empty-seat fallback for the manage-interactable when an upgrade requires a vacant seat ([facility-tiers.md](facility-tiers.md)). |
| **Research lab** | A facility class the player can buy as `factionMisc`; ships at one researcher seat (additional seats land via the lab's own job-site-count tier) | Researcher (worker on the job site) | Short consultative talk — current head, ETA, ~5 newly-eligible researches, cancel current. The dense planner (tree, drag-reorder queue, effect previews, today's overflow tally) opens via the researcher's `"show me the planner"` verb — verb-anchored, the same shape as the secretary's escalation pattern. See [research.md](research.md). |
...
1. **First touch is in person.** The first time the player establishes a relationship with a crew member — hire, first doctrine setting, first standing order — they walk to that person and speak. This is when the order channel is opened.
2. **Quick-recall is via comm.** After first touch, the crew member is reachable from a **comm panel** in the flagship's **captain's office** (see Hubs above): a physical wall of *faces* — portraits, current expression, name, last reported state — one per person the player has met. Click a face → speak to them remotely.

This preserves the diegetic frame (you are still talking to a person, not toggling a slider) while solving the scale problem (you don't have to be physically next to them to give the order). The comm panel is itself a physical object on the wall — visible whether the player uses it or not. It lives in the captain's office rather than on the bridge floor because the bridge is loud, public, and busy with combat stations; private orders happen at the captain's desk.

The comm panel is **not** a fleet roster table. It only shows crew the player has *met and addressed*. Crew who were auto-manned (see below) but never spoken to do not appear; the player has to walk to them once first to promote them from anonymous to addressable.

### Crew assignment: no officer / non-officer split

Crew is undifferentiated in the data model. A ship has N crew slots; each slot is one NPC member of the player's faction with a job tag (gunner, mechanic, comm officer, hangar boss, mess hand, prize-crew lead). There is no "officer" vs. "non-officer" in the type system — there are just NPCs with skills. The player picks who sits at which station; high-skill NPCs become *de facto* officers because they're the ones the player names, talks to, and tracks.

The scale problem (a Pegasus needs ~220 crew) is solved by **officer-led auto-man**:

1. The player **sets the captain** by talking to a candidate (any hired NPC) at the bridge or the captain's office.
2. The player **opens the captain's office**, talks to the captain, and triggers **"man the rest of the ship from the idle pool."**
3. The captain assigns idle hired NPCs from the player's faction roster to vacant crew slots, prioritizing skill-fit per slot type. The player can override per slot by walking to that station (gunner cell, mechanic bay, hangar deck) and talking to the worker on duty — or by talking to the captain about that station via the comm panel.

The first-touch + comm-panel pattern applies symmetrically to anyone manned this way: any crew member the player has spoken to once becomes addressable from the comm panel; unmet crew remain anonymous tallies until the player walks to their station.

So a 220-crew Pegasus is one captain conversation + the four or five named specialists the player picked manually + a wall of anonymous bodies the player can promote into named characters by walking and talking. **Effort scales with the player's interest, not with the crew count.**
...
| **6.0–6.1** | Diegetic recruitment via talk-verb hire on city NPCs. Walkable bridge + walkable hangar + walkable **captain's office** as a per-ship room (single ship; pre-launch readiness summary, no comm panel needed yet). |
| **6.2** | Multi-ship fleet → comm panel as physical object **in the captain's office** (relocated from the bridge floor). **War-room artifact** on the bridge surfaces active-fleet composition (drag tokens between formation and reserve tray), formation slot arrangement, DP commit, route plan. Hired captains stand on the flagship bridge when their ships are docked with the fleet. Doctrine + standing-order verbs surfaced as captain conversations. Fleet roster panel demoted to projection. **Officer-led crew auto-man** verb on the captain (talk to captain → "man the rest from idle pool"). Hangar facility class lands as a real ownable + rentable facility (surface tier in cities, drydock tier at orbital POIs); hangar manager owns receive-delivery, repair-priority, scrap, **transfer-unit-to-other-hangar**, supply/fuel inspection. State-owned hangars at major POIs as the early-game rental escape valve. **Supply-dealer NPC at AE** lands as the supply/fuel inventory replenishment surface (2-day delivery). On-ship hangar deck is the *sortie loadout + forward repair* surface — units at rest live in the hangar facility, not aboard. |
| **6.2.5** | Hangar manager's verbs expand to refit-MS (per-unit retrofit panel still opens by walking up to the MS in the hangar). Pilots stand near their assigned MS in the hangar facility. Secretary's auto-house-undelivered verb lands as the late-game scale valve. **Prisoner verbs** on the brig walk-up (per-cell talk-verb) + on the captain's office comm panel (interrogate / ransom / recruit / execute / hand-over / release); brig-condition stats (food / water / medical) tick on the existing physiology pipeline. |
| **6.3** | Colony command center as the colony management hub; colony administrator owns it. Recruitment post + recruitment officer NPC at colonies. **Colony detention** as brig-overflow target. |
...
- [../fleet.md](../fleet.md) — the data model under the diegetic surface; the player-facing surfaces table is reinterpreted by this file; ship-class interior layout authors the captain's office as a room
- [../sortie.md](../sortie.md) — pre-launch readiness summary at the captain's office is read off this file's MS-state model
- [../post-combat.md](../post-combat.md) — prisoner verbs in the captain's office via comm panel; named POW reveal in the post-combat tally
