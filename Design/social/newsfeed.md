...
- **War-day force-toast hook** `fireWarDayToast()` — authored **inert** in 7.0.A; **now wired in 7.0.B**: `warTransitionSystem` calls it on UC 0079.01.03. The wartime **tone-shift** also shipped in 7.0.B — `rankTopHeadline(list, wartime)` promotes the first war-tagged entry to top-of-broadcast once `isWartime()`, so the bar-TV feed flips to war coverage. See `combat.md § The Phase 7 transition`.
