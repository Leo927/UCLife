...
1. **Hangar manager** — one seat per hangar. The verb surface for everything: receive-delivery, repair-priority, **transfer-unit-to-other-hangar** (paid + delayed; see below), refit (Phase 6.2.5), assemble (Phase 6.3+), scrap, and *supply/fuel inspection* (reads current vs. cap from the facility's stored levels). Per [diegetic-management.md](diegetic-management.md), every owner-side and customer-side fleet verb routes through the manager's talk-verb; the bay floor itself is scenery.
...
A vacant manager seat closes the hangar (no receive-delivery, no repair, no refit, no transfers) — the dim-bay rule applies the same way the dim-shopfront rule does for shops. Workers below quorum reduce throughput linearly; the manager's "any complaints" branch surfaces the gap.

### Supply and fuel storage

Each hangar template declares `supplyStorage` and `fuelStorage` caps. The facility holds the actual reserve; units housed there draw down their `currentSupply` / `currentFuel` to the hangar's pool at end-of-day rollovers. Resupply is **not free** — the hangar manager can show the current pool, but cannot conjure inventory. The pool is filled by **supply-dealer orders** (the supply-dealer NPC class lives at AE Von Braun for POC; other industrial-compound owners host their own dealers in later phases). The dealer's verb is *"order N supply / fuel, deliver to [hangar]"*; shipment arrives ~2 in-game days later (configured per route in `facility-types.json5` as `supplyDeliveryDays`). The secretary's `bulkOrderSupply` verb at the faction office fills every player-owned hangar to cap in one conversation — no per-hangar fine-tuning, accepted as the late-game scale tradeoff.

### Cross-POI unit transit

The hangar manager's **transfer-unit-to-other-hangar** verb sends a unit to any other hangar the player owns or rents. Cost is `transferFee` (configured per route by hangar pair tier in `facility-types.json5`); transit takes `transferDays` in-game days. During transit the unit is unavailable for sortie, refit, or repair — it sits in a transit queue and arrives at the destination at the end of the window. This is the cleanest answer to "I bought the unit at Von Braun but want it at Granada" — the player pays for the courier service through the manager's verb, not by personally ferrying it.
