...
| **6.0 First ship + tactical foundation** | Player buys their first ship. Starsector-shape tactical view (top-down 2D, real-time + pause). Walkable current ship as scene. Earth Sphere continuous-space campaign map opens (Sides 1–7, Luna, Luna II, Earth orbit, asteroid clusters) — see [starmap.md](starmap.md). Single-ship pre-war merc work. | "I burn from Granada to Side 6, dock at a derelict, and decide whether to fight or run" |
| **6.1 Bridge ↔ hangar walk + MS pilot** | Player walks the ship's rooms; physically transits bridge → hangar to climb into an MS. Cockpit primitives now have hostile counterparts. Walking-transit cost on ship AI (leaving bridge mid-combat = AI-piloted ship). | "I leave the bridge mid-fight to take an MS personally — and feel my fleet wobble while I'm gone" |
...
| **6.2 Multi-ship fleet MVP (ships only, no retrofit)** | Two more ship classes (escort + small freighter). Per-ship + crew supply economics (Starsector model). Fleet roster + crew assignment screens. Hire-as-captain / hire-as-crew dialogue branches. Buy-ship dialog at brokers — purchase delivers a ship with its authored loadout, no fitting screen. Mothballing. Doctrine sliders. Persistent fleet damage. Debug "grant fleet" populates a 2-ship fleet + ~30 hired NPCs. Capacity is gated by economics + command bandwidth, not a skill formula. **Ships do not refit** — that depth lives in 6.2.5 on the MS. See [fleet.md](fleet.md). | "My old bartending friend captains my second ship" |
| **6.2.5 MS + pilot + retrofit layer** | The customization-depth phase. `ms-classes.json5`, `ms-weapons.json5`, `ms-frame-mods.json5`. MS runtime entity with per-instance `mountedWeapons` + `frameMods`. Hangar UI, pilot roster + assignment, per-MS supply + per-MS-repair supply economics. **MS retrofit panel** — weapon swap, frame-mod install, role tags. Buy-MS dialog at MS brokers. Auto-assign with manual override. This is where the player tinkers — MS, not ships. | "I bolt the long-range scope onto my GM Cannon and assign it to my second-best `piloting` friend" |
