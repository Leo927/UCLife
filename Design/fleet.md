...
**Hired crew live aboard the flagship (W4.1).** The captain (`assignedCaptainId`) and crew (`crewIds`) are not just roster strings — each is materialized as a walkable `Character` body inside the ship-interior world (`playerShipInterior`), keyed by its stable `npc-crew-<N>` EntityKey, carrying `EmployedAsCrew` + `CrewStation`. Because the interior is the interior of the *currently boarded* hull, the ship world holds exactly the **flagship's** roster as bodies; a reserve ship's crew are "away" and carry no body until the player boards that hull. `systems/crewAboard.ts :: reconcileCrewAboard()` owns this invariant — it runs (via the `ship:crew-reconcile` sim event → `boot/crewAboardBinding.ts`, keeping `sim/` from importing `systems/`) on board, flagship switch, hire, fire, and save-restore, and each call rebuilds the interior from scratch: off-roster bodies removed, missing bodies spawned, a crew hired in the city relocated in via `migrateNpcEntity` (never duplicated across worlds). Firing a crew member removes their body from the ship. **Crew resolve to exactly one world.** Persistence: crew bodies live in the ship world (not the active-scene entity snapshot), so `boot/saveHandlers/crewAboard.ts` reads that world directly and round-trips each body's identity (name, appearance, attributes/skills), then reconciles against the ship handler's restored rosters. The bodies then tick on the same `npcSystem` bucket scheduler as any city NPC.

**Crew duty schedule (W4.1).** Each crew body runs a duty branch gated ahead of the ordinary drives in `ai/trees.ts` (mirroring the `Guard` gate — non-crew fail it instantly). The pure resolver `ai/crewDuty.ts :: resolveCrewDuty(underway, hour)` decides: **underway** (flagship not docked) → man your assigned station; **docked** → report to the **mess** in a meal window, **quarters** in the sleep window, else **off-duty** (the branch fails and the crew drops through to the ordinary vital-driven drives). Station anchors are authored per class as `crewStations` in `ship-classes.json5` and assigned to roster members in order (captain first) by `reconcileCrewAboard`; the shift/meal/sleep windows + arrival radius live in `config/crew.json5`. A crew member on post holds at Action `idle` (a standing watch — a committed kind like `working` would trip `npcSystem`'s `COMMITTED_KINDS` gate and freeze them from re-evaluating when the ship docks or the clock rolls) and records its live duty on `CrewStation.current`. Duty resolution is O(1) per crew per re-think (one dock-state read + a schedule lookup).

**Furnished crew rooms (W4.2).** The crew quarters + mess are real, usable rooms — not empty labelled boxes. Each class authors `interactables` in `ship-classes.json5`: the crew quarters carry a rack per crew (the captain's bunk + N crew bunks, one per tile) and the mess carries an `eat` station. Bunks are the rent-free **`bunk`** `Bed` tier (attached to the `sleep` kiosk at spawn in `ecs/spawn.ts`); the player **free-claims** one on click (a ship-bunk branch in `systems/interaction.ts` — no realtor lease, unlike a city bed) and off-duty crew claim one via the ordinary `findBestOpenBed` drive. The mess `eat` station recovers hunger for whoever uses it; when a crew member reports to the mess in a meal window, the new `eatAtMess` BT action (`ai/agent.ts`) walks them to the station and consumes **one meal from the shared ship supply pool** (`FleetPool.supplyCurrent`, drawn down `config/crew.json5 → mess.mealSupplyCost` per sitting) — *not* personal `Inventory.meal`; when the pool can't cover a meal the crew stand at the mess but don't eat. This is what turns the Task 2 "walk to the room and idle" placeholders into rooms crew and player actually live in.
...
> **Shipped (W4.3, completes W1.5).** `systems/onShipRepair.ts :: runOnShipRepair(ship)` runs on the daily repair chain (`boot/hangarRepairTick.ts`, alongside `hangarRepairSystem`) and clamps the aboard-MS repair to `[onShipRepairFloor, onShipRepairCap]` — the two ship stats declared at 6.2.B but until now read nowhere. Aboard = `Ms.storedOnShipKey === flagshipKey` AND `Ms.dockedAtPoiId === ''` (the custody invariant); depoted MS keep going to `hangarRepair.ts`. A below-floor MS is refused (sidelined); an in-band MS is patched armor-first then hull up to `cap × max` and no further. `damageState` routes only through `computeMsDamageState`, so an aboard MS stays `ready`-with-deficit (never `in-repair`). Repair rate is `sortie.json5 → onShipRepair.pointsPerDay`; the band cap/floor project from the ship template (`ship-classes.json5`, config-default fallback) onto the `ShipStatSheet`. `describeOnShipRepair(ship)` is the read model for the hangar-deck panel (W4.3b). The **hangar boss** is the crew member whose duty station is the `hangarBay` room (`crewAboard.ts :: isHangarBossCrew`); `DialogueRoles.isHangarBossOnDuty` gates the hangar-deck talk surface.
...
| **6.2.5** | **MS depth phase — sliced by user story.** Three sub-phases land sequentially: **6.2.5.A** (see + customize my starter MS), **6.2.5.B** (multi-MS roster: pilots, broker, depot), **6.2.5.C** (in-tactical sortie loop). Each ships a complete diegetic loop end-to-end; the tree is never half-built between merges. Sub-phase slicing in [§ 6.2.5 sub-phase slicing](#625-sub-phase-slicing). Tangential 6.2.5-era work that does **not** belong to the MS user-story arc — **prisoner verbs** on the brig walk-up + captain's office comm panel (interrogate / ransom / recruit / execute / hand-over / release) + in-flight prisoner upkeep via brig-condition stats — ships on its own slice alongside, not as part of A/B/C. See [sortie.md](sortie.md), [post-combat.md](post-combat.md). |
...
| **6.2.G** ✅ | **Mothball + transfer-to-other-hangar.** Mothball verb on the roster cell (toggles `mothballed`, kills drain + salary, removes from active fleet, clears formation slot + captain Effect; flagship + in-transit ships refuse the toggle). Hangar manager's `transfer-to-other-hangar` verb (paid + delayed): destination cap-gated by `hangarSlotClass`, fee resolved per directed route via `fleetConfig.transferFees` + the existing `transitFee`, calls into `enqueueShipTransit` so the daily lander tick (6.2.E2 infra) handles arrival. Refuses to transfer mothballed / in-transit / flagship ships. Hire branches hide mothballed hulls from the picker. Save round-trip persists `mothballed`. | Mothball a ship → daily supply drain drops; transfer the light ship from VB → Granada. | 6.2.C2, 6.2.F |
| **6.2.H** ✅ | **Debug "grant fleet" function** — single-click orchestrator on the dev panel: grants ¥10,000,000, buys + receives a `lunarMilitia` (VB) + `pegasusClass` (Granada drydock) via the existing C1/C2 delivery pipeline, stocks both hangars to max supply + fuel, spawns 30 procedural NPCs at VB, hires captains on both new ships, fills crew via `manRestFromIdlePool`, and promotes the Pegasus into the active fleet at the first free slot. Idempotent — a second click is refused with `already_granted`. Flagship-captain seat is left empty by design (the flagship represents the player on the bridge). | One click → end-to-end fleet ready to exercise the downstream phases. | All above |
| **6.2.A.3** ✅ | **Granada drydock mini-station — concourse service buildings.** The `granadaDrydock` concourse gains three hand-placed `fixedBuildings` so the live-in crew have somewhere to drink and get patched up, and the player can order hangar supply on-site: `drydockBar` / `drydockClinic` (fixed-size variants of the procgen `bar` / `clinic` — identical `open_floor` layout + templates, fixed `{w,h}` size so the `scenes.ts` validator accepts them as fixedBuildings) and `supplyDepot` (a small standalone shell for the AE supply-dealer kiosk that previously existed only inside the `aeComplex` crafted layout). A second AE supply-dealer special NPC seats at the depot — reuses the `ae_supply_dealer` specId with no collision, since koota gives each scene its own world and `spawnSpecialNpcs` processes each NPC in its declared scene. Bar + clinic auto-staff from the scene's replenishment pool; `replenishment.target` stays 12 — it tracks the job count (hangar + bar + clinic), not standing population. | Ride the lift to Granada → the concourse has a bar, a clinic, and a supply depot; order supply for the drydock hangar from the on-site AE dealer. | 6.2.A.2, 6.2.F |

**All Phase 6.2 sub-phases (B, C1, C2, D, E1, E2, F, G, H) are now shipped.** The fleet POC is end-to-end exercisable from a fresh save: buy two hulls, hire captains, stock supplies, mothball / transfer / launch into active fleet, persist damage across encounters, repair through hangar throughput. The downstream depth phase (6.2.5 — MS + pilots + retrofit) is the next chunk.
...
## 6.2.5 sub-phase slicing

The previous 6.2.5 fat-bullet bundled three orthogonal player-facing loops that don't ship coherently in one PR — the *core diegetic shape* (see + customize one MS), the *scale layer* (multiple MS, pilots, depot, broker), and the *in-tactical depth* (per-MS sortie resources, doors, mid-combat resupply). Re-sliced by user story so each slice ships a complete loop end-to-end. Same discipline as 6.2: each row below is intended as a separate PR / commit that demos a real thing without leaving half-built UX in the tree between merges.

**Diegetic shape (locked across all three slices):** an owned MS sits in a ship's hangar bay as a walkable sprite. Walking up to the MS itself is the climb-in interaction (replacing 6.1's floating `climbIntoMs` kiosk). A **terminal** spawns adjacent to each MS sprite — that terminal is the surface for all retrofit + management verbs. Same shape as the captain's office (`captainsDesk` + `commPanel` paired interactables): the body is the sprite, the verb wall is the terminal.

| # | Slice | Demo | Depends on |
|---|---|---|---|
| **6.2.5.A** ✅ | **See + customize my starter MS.** End-to-end core loop at minimum content. MS as persistent ECS entity + save handler (mirror of 6.1.5's Ship plural pivot). Player starts with one MS auto-housed in the flagship's hangar bay (no broker required for this slice — flagship arrives with starter MS the way it arrives with a configured captain seat). MS sprite renders in the `hangarBay` room, positioned by bay-slot index. Per-MS **terminal** interactable spawns adjacent to the sprite. `climbIntoMs` is no longer a fixed kiosk — it's the MS sprite itself, and reads from the actual MS instance (kills the hardcoded `PLAYER_MS_CLASS_ID` in `cockpit.ts`). Terminal opens a retrofit panel with one verb: **swap weapon** (compatibility by hardpoint size, pulled from a small starter parts inventory the player owns from day one). Pilot defaults to player; explicit pilot roster deferred. `ms-classes.json5` formalized; `ms-weapons.json5` as a separate data file (today shared with ship weapons). `MsStatSheet` + `MsEffectsList` traits + save round-trip. | "I walk into my flagship's hangar, see my GM standing there, walk up to its terminal, swap the rifle for a beam cannon, then walk back to the MS and climb in for the next sortie." | 6.2.0 |
| **6.2.5.B** ✅ | **Multi-MS roster: pilots, broker, depot.** AE vehicle (fighter / MW) broker NPC + `aeVehicleSales` dialogue branch (the AE *MS* broker proper is faction/research-gated and lands later; this slice ships the fighter frame `civFighter` as the first public 0077 catalog entry). New `MsDeliveryRow` + `msDeliverySystem` parallel to the 6.2.C1 ship pipeline. `Ms` trait grows `dockedAtPoiId` / `pilotId` / `transitDestinationId` / `transitArrivalDay`; MS sprite + terminal visible at surface and drydock hangars via `refreshDepotMsLayout`. `EmployedAsPilot` trait + `hireAsPilot` branch on civilian dialog; pilot salary folds into `factionSalarySystem`. Auto-assign by `piloting` skill on delivery receive (full roster panel + reassign-from-terminal deferred — the auto-assign + the existing MS terminal cover the smoke surface). **Transfer-MS-between-hangars** verb on hangar manager + `msTransferFees` / `msTransitDays` in `fleet.json5`. Per-MS `supplyPerDay` authored on `ms-classes.json5` (drain integration is a small follow-up PR). Frame mods + AE MS broker + MS-parts combat loot + AE parts broker explicitly deferred to keep the slice end-to-end exercisable. | "I hire a pilot at the bar, buy a fighter at the AE broker, park it in the VB state hangar, then transfer it onto the Pegasus before the next sortie." | 6.2.5.A |
| **6.2.5.C** | **In-tactical sortie loop with resources.** The depth that turns sortie play from "fly until you die" into "manage your relaunch cycle." Per-MS `currentPropellant` + `currentAmmoByWeapon` + `currentLifeSupport` stats; `propellantStorage` + `lifeSupportMinutes` as MS template stats. Per-ship-class **`hangarDoors[]`** authoring + door queueing + spawn-at-door-position (kills the 6.1 single-placeholder-door simplification). **Mid-combat resupply protocol** — full `baseResupplySec / hangarBoss.workPerformance / (1 + Σ mechanicCrewEfficiency) / resourceBoostMul` formula from [sortie.md](sortie.md#mid-combat-resupply-protocol). Stranded-MS recovery tug (`computers + mechanics` skill check; occupies a door for the duration). Frame mods land here too — bolt-on items that consume frame slots and emit Effects against MS stats (extended propellant tank, autoloader, life-support pod, armor plating). | "I dock my dry GM mid-fight, the resupply timer ticks, I take the helm — fifteen seconds later the log says she's hot and I sprint back down the hall to climb back in." | 6.2.5.B |

### Ordering rationale

- **A before B** — the core diegetic shape (MS sprite, terminal, retrofit panel surface) is load-bearing for everything downstream; shipping multi-MS scale before the single-MS surface is right is the express path to inconsistent UX. A locks the shape with one MS; B replicates it.
- **B before C** — sortie-loop depth (propellant, ammo, life-support, door queueing) reads at multi-MS scale, where managing relaunch order across a wing matters. Adding it on a one-MS roster is theatre.
- **Prisoner verbs ship independently.** They share the 6.2.5 era but not the user-story arc — no shared data model, no shared UI surface. Treating them as a fourth MS slice would over-slice and invite the inconsistency the user-story shape exists to prevent.
- **Frame mods land in C, not B** — frame mods that *only* modify hull/armor/topSpeed (no sortie-resource impact) could land in B, but the design intent is mods that emit against sortie stats (autoloader, extended propellant tank, life-support pod). Folding them in with C keeps the Effect channel and its consumers shipping together.
- **No slice promises new data authoring *and* new ECS systems *and* new UI in one PR.** A is the biggest and stretches it (MS entity + sprite + terminal + retrofit panel); a follow-up slice may split A into A.1 (entity + save) and A.2 (sprite + terminal + retrofit) if scoping during build reveals it's too wide.

### Deferred from 6.2.5.B (small additive follow-ups)
...
- **AE MS-parts broker NPC** — ✅ (Issue #64) `ae_parts_dealer_vb` special NPC + desk in the VB airport lobby + `aePartsSales` branch (weapons + frame mods). Parallel to the ship/vehicle pattern, but parts are crates: a buy debits `Money` and credits `PlayerPartsInventory` immediately (no delivery queue / hangar slot). Catalog ids + linear price-derivation constants live in `fleet.json5` (`partsSalesCatalog` / `partsPricing`); the shared `buyPart()` in `systems/partsSales.ts` runs the transaction for both the branch and the `buyPartCheat` handle. Public 0077 catalog = tier-1 parts only.
- **MS-parts combat loot.** ✅ (Issue #64) Per-enemy-class `salvage[]` on `enemyShips.json5` rows (`{ partId, kind: 'weapon' | 'frameMod', chance, qty }`) — authored on the per-class file rather than `space-entities.json5` instances. Rolled with the seeded combat RNG in `onEnemyDestroyed` (accumulated per engagement, reset at `startCombat`), drained at `endCombat('victory')` into `PlayerPartsInventory`; the post-combat tally grows a salvaged-parts section. `breakDownEnemiesForVictory()` drives the canonical destruction path for deterministic smoke loot.
- **`mobileWorker` frame** — ✅ (Issue #65) second public catalog entry (civilian MW). The `vehicleSalesRepCatalog` entry grew from one MS class to a list (`{ msClassIds }`) and `aeVehicleSales.tsx` now renders one selectable row per class, so the VB broker is a two-row catalog (`civFighter` + `mobileWorker`).
- **Full pilot roster panel** — ✅ (Issue #65) at the captain's-office desk (`PilotRosterPanel`, sibling of `FleetRosterPanel`). Lists every pilot with `piloting` level + assignment + `idle / assigned / damaged` state and reassigns in one click. NB: the MS retrofit panel never actually shipped a reassign-pilot verb, so the roster is the **first** manual-reassign surface; the write routes through the new `assignPilotToMs` in `systems/msPilotAssign` (reusing `clearPilotAssignment` for both sides) so manual reassign and the receive-MS auto-assign stay one source of truth.
