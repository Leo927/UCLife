...
| **6.2.5.B** | **Multi-MS roster: pilots, broker, depot.** Now the player can grow an MS roster across multiple ships and a depot. AE fighter / MW broker NPC + buy-vehicle dialog (AE MS broker proper is faction/research-gated; fighter + MW frames are public in 0077). Vehicle-delivery pipeline reusing the 6.2.C1 `ShipDeliveryRow` shape. MS / vehicle sprite + terminal visible in surface and drydock hangars (depot storage, not just on-ship). Pilot hire branch on civilian NPC dialog (`hireAsPilot`). Pilot roster panel + auto-assign by `piloting` skill + reassign-from-terminal. **Transfer-MS-between-hangars** verb on the hangar manager (paid + delayed, reuses 6.2.G's transit pipeline). Per-MS supply + per-MS-repair supply economics fold into the existing daily drain. **MS-parts loot** via per-class salvage table (combat-side parts acquisition feeds the broker-side inventory). Frame mods deferred to keep this slice scoped to *quantity*, not *depth-per-MS*. | "I hire a pilot at the bar, buy a second GM at the AE broker, park it in the state hangar, then transfer it onto the Pegasus before the next sortie." | 6.2.5.A |
