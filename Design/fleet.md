...
| **6.2.A** | **Hangar facility class + state-rental hangars.** `hangar` facility-type in `facility-types.json5` (surface + orbital tiers; manager + worker job sites; capacity-only verbs). State-owned rental hangar at Von Braun (surface) and Granada drydock (orbital, walkable via orbital-lift interactable). Existing flagship hangar slot migrates into the new facility shape. | Walk to the Von Braun state hangar, see the manager + capacity counts. | 6.1.5 |
| **6.2.B** | **`ShipStatSheet` + `ShipEffects` + persistent fleet damage + repair throughput.** Project ship-class scalars (`hullPoints`, `armorPoints`, `topSpeed`, …) into a per-ship stat sheet; reuse the character/faction Effects engine; save round-trip. Hangar daily-throughput formula + repair-priority verb on the manager. Damage no longer evaporates at dock. | Take damage in combat → dock → damage persists → wait N days → manager repair-priority finishes restoration. | 6.2.A |
| **6.2.C1** | **Light-hull buy at the AE Von Braun spaceport.** One light ship class authored. AE sales rep NPC at the spaceport. Buy-ship dialog with explicit capacity gating ("no slot — rent at Von Braun state hangar"). Delivery queue + receive-delivery verb on hangar manager. | Click buy → manager exposes receive-delivery → ship appears in a slot. | 6.2.A, 6.2.B |
| **6.2.C2** | **Pegasus-class buy at the Granada drydock + fleet roster screen.** Second ship class (capital). AE rep at the Granada drydock. Fleet-roster read-mostly notebook surface — lists every owned ship + state + captain. Cell-level mothball / scrap stubs (mothball wired at 6.2.G). | Buy a capital → orbital lift → drydock → receive-delivery; roster shows two ships. | 6.2.C1 |
| **6.2.D** | **Hire-as-captain / hire-as-crew branches + crew assignment + officer auto-man.** Two hire branches on NPC dialog. Crew assignment screen (move / hire / fire). Captain's talk-verb "man the rest from idle pool" supersedes the existing auto-pull stub. | Hire 5 idle NPCs → walk to the second ship's captain → "man the rest" fills `crewRequired`. | 6.2.C2 |
| **6.2.E1** | **War-room plot table on flagship bridge + `IsInActiveFleet` + aggression doctrine.** War-room interactable; drag tokens between active grid + reserve tray; per-ship aggression slider (cautious / steady / aggressive). Roster's active/reserve column reads the flag (verb lives at the war room only). | Drag the second ship into the formation grid → roster reflects active state. | 6.2.D |
...
| **6.2.F** | **Supply + fuel economy.** Supply dealer NPC at the AE industrial compound. `order-supply` / `order-fuel` verbs with 2-day delivery (configurable per route). Per-hangar `supplyStorage` / `fuelStorage` caps. Daily aggregate supply drain across non-mothballed fleet. Campaign HUD shows fleet supply. Secretary's bulk-order verb at the faction office. | Daily drain ticks down storage → order supply at the dealer → 2-day delivery lands in the target hangar. | 6.2.A |
...
- **Transfer-pipeline ownership floats** between 6.2.E2 and 6.2.G: whichever ships first defines the shipment-day infra (`transferDays` in `facility-types.json5`, the `transit*` instance fields); the other adds its user-facing surface. Both depend on 6.2.A's hangar manager.
